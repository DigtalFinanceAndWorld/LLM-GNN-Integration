...
flags.DEFINE_string("data_dir", "/home/a/zmb_workspace/dataset/ZipZap", "data directory.")
...
def load_data(f_in, f_out):
    ...
    while True:
        ...
        if from_address == "" or to_address == "":
            ...
            continue
        try:
            eoa2seq_in[to_address].append(
                [from_address, block_number, block_timestamp, value, "IN", 1])  # not process trans
        except:
...
def seq_duplicate(eoa2seq_in, eoa2seq_out):
    ...
    eoa_list = list(
        eoa2seq_agg_out.keys())  # eoa_list must include eoa account only (i.e., have out transaction at least)
    eoa2seq_agg = {}
...
def main():
    ...
    print("Add normal " + FLAGS.dataset)
    ...
    with open("../data/eoa2seq.pkl", "wb") as f:
