...
#
# print("CUDA version:", tf.sysconfig.get_build_info().get('cuda_version', 'Unknown'))
# print("cuDNN version:", tf.sysconfig.get_build_info().get('cudnn_version', 'Unknown'))
# print("CUDA Library path:", tf.sysconfig.get_build_info().get('cuda_library_dir', 'Unknown'))
# print("cuDNN Library path:", tf.sysconfig.get_build_info().get('cudnn_library_dir', 'Unknown'))
#
...
flags.DEFINE_integer("max_seq_length", 29, "")
...
flags.DEFINE_integer("epoch", 3, "")
flags.DEFINE_float("learning_rate", 1e-4, "")
flags.DEFINE_integer("num_train_steps", 10000, "Number of training steps.")
flags.DEFINE_integer("num_warmup_steps", 50, "Number of warmup steps.")
flags.DEFINE_integer("save_checkpoints_steps", 2000, "")
flags.DEFINE_integer("iterations_per_loop", 1000, "How many steps to make in each estimator call.")
flags.DEFINE_integer("max_eval_steps", 100, "Maximum number of eval steps.")
flags.DEFINE_integer("neg_sample_num", 1000, "The number of negative samples in a batch")
flags.DEFINE_string("neg_strategy", "zip", "Strategy of negative sampling")
flags.DEFINE_bool("use_tpu", False, "Whether to use TPU or GPU/CPU.")
flags.DEFINE_string("data_dir", None, "data dir.")
flags.DEFINE_bool("cross_share", True, "whether to share or not")
flags.DEFINE_bool("load_cross", False, "whether to load from cross")
flags.DEFINE_string("vocab_filename", "vocab", "vocab filename")

flags.DEFINE_string(
    "bert_config_file", "./zipzap_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string(
    "train_input_file", f"{FLAGS.data_dir}/train.tfrecord",
    "Input TF example files (can be a glob or comma separated).")

flags.DEFINE_string(
    "test_input_file", f"{FLAGS.data_dir}/test.tfrecord",
    "Input TF example files (can be a glob or comma separated).")

flags.DEFINE_string(
    "checkpointDir", f"{FLAGS.data_dir}/ckpt_dir",
...
def model_fn(features, mode, bert_config, vocab, init_checkpoint, learning_rate,
    ...
    embedding_table_list, factorize_table_list = model.get_embedding_table()

    (masked_lm_loss,
     masked_lm_example_loss, masked_lm_log_probs) = get_masked_lm_output_negative_sampling(
        bert_config,
...
def get_masked_lm_output_negative_sampling(bert_config, input_tensor,
                                           embedding_table_list, factorize_table_list,
                                           positions, label_ids, label_weights, vocab):
    """Get loss and log probs for the masked LM."""

    if FLAGS.neg_strategy == "uniform":
        neg_ids, _, _ = tf.nn.uniform_candidate_sampler(true_classes=[[len(vocab.vocab_words) - 1]],
                                                        num_true=1,
                                                        num_sampled=FLAGS.neg_sample_num,
                                                        unique=True,
                                                        range_max=len(vocab.vocab_words))

    elif FLAGS.neg_strategy == "zip":
        neg_ids, _, _ = tf.nn.log_uniform_candidate_sampler(true_classes=[[len(vocab.vocab_words) - 1]],
                                                            num_true=1,
                                                            num_sampled=FLAGS.neg_sample_num,
                                                            unique=True,
                                                            range_max=len(vocab.vocab_words))
        ...
        neg_ids, _, _ = tf.nn.fixed_unigram_candidate_sampler(true_classes=[[len(vocab.vocab_words) - 1]],
                                                              num_true=1,
                                                              num_sampled=FLAGS.neg_sample_num,
                                                              unique=True,
                                                              range_max=len(vocab.vocab_words),
    ...
    neg_ids = tf.cast(neg_ids, tf.int32)
    neg_ids = neg_ids + 1 + 3  # + 4 (1 padding, 2 mask, 3 not use)
    ...
    with tf.variable_scope("cls/predictions"):
        ...
        pos_output_weights = modeling.new_embedding_lookup(label_ids,
                                                           bert_config.bucket_list,
                                                           embedding_table_list,
                                                           factorize_table_list)

        pos_output_weights = tf.reshape(pos_output_weights, [-1, pos_output_weights.shape[-1]])

        neg_ids = tf.expand_dims(neg_ids, axis=0)
        neg_output_weights = modeling.new_embedding_lookup(neg_ids,
                                                           bert_config.bucket_list,
                                                           embedding_table_list,
                                                           factorize_table_list)

        neg_output_weights = tf.reshape(neg_output_weights, [-1, neg_output_weights.shape[-1]])
...
def main(_):
    ...
    # load vocab
    vocab_file_name = os.path.join(FLAGS.data_dir, FLAGS.vocab_filename)
    ...
    if FLAGS.do_train:
        ...
        with tf.Session(config=config) as sess:
            ...
            # start = time.time()
            start = timer()
            while iter <= FLAGS.num_train_steps:
                try:
                    ...
                    if iter % 1000 == 0:
                        ...
                        loss = np.mean(losses)
                        print("iter=%d, loss=%f, time=%.3fs" % (iter, loss, end - start))
                        losses = []
                        # start = time.time()
                        start = timer()
        ...
        if FLAGS.init_checkpoint == None:
            raise ValueError("Must need a checkpoint for evaluation.py")
            ...
            while True:
                try:
                    ...
                    if iter % 500 == 0:
                        ...
                        print("iter=%d, time=%.3fs" % (iter, end - start))
                        # start = time.time()
                        start = timer()
                    iter += 1
