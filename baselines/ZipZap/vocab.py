...
class FreqVocab(object):
    ...
    def generate_vocab(self):
        ...
        self.special_tokens = ["[MASK]", "[pad]", '[NO_USE]', '[UNK]']
    ...
    def convert_tokens_to_ids(self, tokens):
        return convert_by_vocab_with_unk(self.token_to_ids, tokens)
...
def convert_by_vocab_with_unk(vocab, tokens):
    """Converts a sequence of [tokens|ids] using the vocab, using [UNK] for unknown tokens."""
    output = []
    unk_id = vocab.get('[UNK]')  # 获取 UNK 标记的 ID
    for token in tokens:
        if token in vocab:
            output.append(vocab[token])
        else:
