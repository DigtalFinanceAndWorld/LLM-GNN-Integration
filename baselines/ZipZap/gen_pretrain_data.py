...
HEADER = ('hash,nonce,block_hash,block_number,transaction_index,from_address,to_address,value,gas,gas_price,input,'
          'block_timestamp,max_fee_per_gas,max_priority_fee_per_gas,transaction_type').split(
    ",")
...
def main():
    vocab = FreqVocab()
    print("===========Load Sequence===========")
    train_data = f"{FLAGS.data_dir}/train_eoa2seq.pkl"
    with open(train_data, "rb") as f:
        train_eoa2seq = pkl.load(f)
    print(f"Train data: {train_data}")
    print("Number of train accounts:", len(train_eoa2seq))

    test_data = f"{FLAGS.data_dir}/test_eoa2seq.pkl"
    with open(test_data, "rb") as f:
        test_eoa2seq = pkl.load(f)
    print(f"Test data: {test_data}")
    print("Number of test accounts:", len(test_eoa2seq))

    vocab.update(train_eoa2seq)
    vocab.update(test_eoa2seq)

    vocab.generate_vocab()

    vocab_file_name = os.path.join(FLAGS.data_dir, FLAGS.vocab_filename)
    print("Saving vocab to:", vocab_file_name)
    with open(vocab_file_name, 'wb') as output_file:
        pkl.dump(vocab, output_file, protocol=2)

    print("Token size:", len(vocab.vocab_words))

    print("===========Original===========")
    length_list = []
    for eoa in train_eoa2seq.keys():
        seq = train_eoa2seq[eoa]
    ...
    if FLAGS.total_drop:
        train_eoa2seq = total_repeat_drop(train_eoa2seq)
    ...
    seqs = []
    idx = 0
    for eoa, seq in train_eoa2seq.items():
    ...
    if FLAGS.do_embed:
        ...
        output_filename = os.path.join(FLAGS.data_dir, "embed.tfrecord")
    ...
    if FLAGS.do_eval:  # select 20% for testing
        ...
        eval_output_filename = os.path.join(FLAGS.data_dir, "test.tfrecord")
    ...
    # 打印 normal_instances 的类型和长度
    print(f"Type of normal_instances: {type(normal_instances)}")
    print(f"Length of normal_instances: {len(normal_instances)}")

    # 打印 normal_instances 的前几个元素，假设 normal_instances 是一个列表
    if len(normal_instances) > 0:
        print("First few elements of normal_instances:")
        for i, instance in enumerate(normal_instances[:5]):  # 打印前5个元素
            print(f"Instance {i}: {instance}")
            
    write_instance = normal_instances
    rng.shuffle(write_instance)

    output_filename = os.path.join(FLAGS.data_dir, "train.tfrecord")
    ...
    write_instance_to_example_files(write_instance, FLAGS.max_seq_length,
                                    MAX_PREDICTIONS_PER_SEQ, vocab,
                                    [output_filename])
    print("gen_pretrain_data Finished..")
