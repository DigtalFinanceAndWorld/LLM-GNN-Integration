...
def load_finetune_data(data_dir, file_prefix):
    phisher_account = pd.read_csv(f"{data_dir}/{file_prefix}_phisher_account.txt", names=["account"])
    phisher_account_set = set(phisher_account.account.values)

    with open(f"{data_dir}/{file_prefix}_eoa2seq.pkl", "rb") as f:
        eoa2seq = pkl.load(f)

    return phisher_account_set, eoa2seq


def is_phish(address, phisher_account_set):
    return 1.0 if address in phisher_account_set else 0.0


def process_finetune_eoa2seq(eoa2seq, phisher_account_set, max_num_tokens, sliding_step):
    seqs = []
    label_list = []
    idx = 0
    ...
    for eoa, seq in eoa2seq.items():
        if len(seq) <= max_num_tokens:
            ...
            seqs[idx] += seq
            label_list.append(is_phish(eoa, phisher_account_set))
            idx += 1
    ...
    return seqs, label_list


def main():
    train_phisher_account_set, train_eoa2seq = load_finetune_data(FLAGS.data_dir, "train")
    test_phisher_account_set, test_eoa2seq = load_finetune_data(FLAGS.data_dir, "test")

    with open(FLAGS.vocab_file_name, "rb") as f:
        vocab = pkl.load(f)

    print("number of train account:", len(train_eoa2seq))
    print("number of test account:", len(test_eoa2seq))

    train_seqs, train_label_list = process_finetune_eoa2seq(train_eoa2seq, train_phisher_account_set,
                                                            FLAGS.max_seq_length - 1, SLIDING_STEP)
    test_seqs, test_label_list = process_finetune_eoa2seq(test_eoa2seq, test_phisher_account_set,
                                                          FLAGS.max_seq_length - 1, SLIDING_STEP)
