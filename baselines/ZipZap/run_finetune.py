...
flags.DEFINE_string("index", None, "data index")
flags.DEFINE_string("last_index", None, "data last_index")
flags.DEFINE_string("result_dir", None, "result dir.")
flags.DEFINE_string("init_checkpoint", None,
                    "Initial checkpoint (usually from a pre-trained BERT model).")
flags.DEFINE_bool("use_tpu", False, "Whether to use TPU or GPU/CPU.")
flags.DEFINE_string("vocab_file_name", None, "vocab filename")
...
flags.DEFINE_string("train_input_file", f"{FLAGS.data_dir}/finetune_train.tfrecord",
                    "Input train file for finetuning")
flags.DEFINE_string("test_input_file", f"{FLAGS.data_dir}/finetune_test.tfrecord", "Input test file for finetuning")
...
def main(_):
    # load label
    phisher_account = pd.read_csv(f"{FLAGS.data_dir}/test_phisher_account.txt", names=["account"])
    ...
    # load vocab
    with open(FLAGS.vocab_file_name, "rb") as f:
        vocab = pkl.load(f)
    ...
    train_bert_model, train_op, total_loss = model_fn(train_features, mode, bert_config, vocab,
                                                      ...
                                                      FLAGS.learning_rate,
                                                      FLAGS.num_train_steps, FLAGS.num_warmup_steps, FLAGS.load_cross,
                                                      False)
    ...
    while True:
        try:
            address_id_v, y_hat_v, label_v, loss = sess.run(
                [test_features["address"], y_hat, test_features["label"], total_loss])
    ...
    test_id = np.array(address_list)
    test_y = np.array(agg_label_list)
    test_pred = np.array(agg_y_hat_list)
    ...
    result_df.to_csv(f'{FLAGS.result_dir}/result_{FLAGS.index}.csv', index=False)
    print("Test results saved to 'result_{index}.csv'")
    result_csv = f"{FLAGS.result_dir}/result.csv"

    if FLAGS.index == "1":
        create_csv(test_y, test_pred, result_csv)
    elif FLAGS.index == FLAGS.last_index:
        add_and_evaluation(test_y, test_pred, result_csv, FLAGS.index)
    else:
        add_data(test_y, test_pred, result_csv, FLAGS.index)

    print("run_finetune Finished..")
