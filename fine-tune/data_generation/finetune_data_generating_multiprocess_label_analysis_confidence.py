...
def process_node(node, graph, labels, gpt_response):
    ...
    print(f"node: {node}, confidence: {confidence}")
    conclusion = ""
    if label == 0:
        conclusion = (
...
def main():
    ...
    for multiple in multiple_list:
        ...
        result_dir = f'../../dataset/finetune/finetune_by_label_analysis_confidence/'
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i, index in enumerate(sorted_dataset_list):
                ...
                gpt_response = get_gpt_response_by_label(analysis_file_path)
                # 将任务提交到进程池
