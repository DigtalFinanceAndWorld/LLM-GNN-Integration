...
def process_node(node, graph, labels, gpt_response):
    ...
    if label == 0:
        conclusion += "\nAfter analysis and discussion, we will temporarily mark it as a Non-Phishing node. The label of this node is 0."
    elif label == 1:
        conclusion += "\nAfter analysis and discussion, we will mark it as a Phishing node. The label of this node is 1."
    else:
        print("error label")
...
### Objective
Accurately identifying phishing nodes is critical to protecting the financial ecosystem. Use the provided data to identify **positive features** (indicative of phishing) and **negative features** (indicative of non-phishing) for the Target Node. Analyze the transaction behavior of the Target Node to classify it as either a Phishing Node (label 1) or a Non-Phishing Node (label 0). Evaluate both positive (phishing-like) and negative (non-suspicious) features in the transaction data, then provide a final label based on a balanced assessment of these features. Provide a concise analysis that includes positive and negative features, and ends with a clear, definitive answer in this format:
...
def main():
    ...
    for multiple in multiple_list:
        ...
        result_dir = f'../../dataset/finetune/finetune_cot_1_label_logits/'
