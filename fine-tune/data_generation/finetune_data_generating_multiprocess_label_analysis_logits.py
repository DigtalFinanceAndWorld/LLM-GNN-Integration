...
from select_analysis import get_analysis
...
def process_cluster(index, dataset_name, delay, dataset_dir, queue, analysis_dict):
    ...
    for node in graph_train:
        if node in analysis_dict:  # 检查 node 是否在 analysis_dict 中
            analysis = analysis_dict[node]  # 获取对应的 value
...
def main():
    ...
    for multiple in multiple_list:
        ...
        sorted_dataset_list = sorted_dataset_list[8:15]
        print(f"Dataset_list: {sorted_dataset_list}")

        result_dir = f'../../dataset/finetune/finetune_label_analysis_logits_4096/'
