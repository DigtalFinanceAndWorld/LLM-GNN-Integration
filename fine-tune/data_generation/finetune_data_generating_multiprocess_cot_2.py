...
from select_analysis import get_cot_2
...
def process_node(node, graph, labels, gpt_response):
    ...
    score = int(gpt_response['score'])
    if label == 0:
        if score < 20:
            label = "\"Non-Phishing\""
            print(f"node: {node}, Label: {label}")
            conclusion = (
                f"\nAfter analysis and discussion, we will mark it as a Non-Phishing node."
            )
        else:
            label = "\"uncertain\""
            print(f"node: {node}, Fraudulent Probability: {score}")
            conclusion = (
                f"\nAfter analysis and discussion, the classification of this node is uncertain. "
                f"While it exhibits some indicators of typical behavior, there are also suspicious patterns. "
                f"As a result, we estimate the Fraudulent Probability of this node is {score}."
            ) 
    elif label == 1:
        if score > 80:
            label = "\"Phishing\""
            print(f"node: {node}, Label: {label}")
            conclusion = (
                f"\nAfter analysis and discussion, we will mark it as a Phishing node."
            )
        else:
            label = "\"uncertain\""
            print(f"node: {node}, Fraudulent Probability: {score}")
    ...
    return {
        "instruction": """
### Instruction
You are a blockchain analyst specializing in identifying fraudulent behavior within the Ethereum network. Your task is to analyze the transaction data of a **Target Node** and classify it as either a **Phishing Node** or **Non-Phishing Node**. This is a **binary classification problem**, but if the conclusion is uncertain, provide a **Fraudulent Probability** indicating the likelihood of phishing behavior.
...
**1. Strong Confidence (Certain Label Phishing or Non-Phishing)**: If the analysis reveals multiple suspicious patterns strongly indicative of phishing or non-phishing behavior, confidently classify the node as:
   - Phishing Node (if multiple high-risk indicators are present)
   - Non-Phishing Node (if typical patterns are seen and few suspicious attributes are detected)

**2. Uncertain Classification (Fraudulent Probability)**: If the data is ambiguous or only partially matches phishing indicators, provide a Fraudulent Probability (0–100) to represent the likelihood of phishing. In such cases:
   - Detail both phishing and non-phishing indicators.
   - Reason through the conflicting evidence, using reasoning to explain why the Probability falls in a particular range.

### Objective
Accurately identifying phishing nodes is critical to protecting the financial ecosystem. Use the provided data to assess the likelihood of phishing behavior, and give a reasoned response in this format:
...
### Output
{{"Label": {label}, "Fraudulent Probability": {score}}}
### Analysis
{analysis} {conclusion}
"""
    }
...
def main():
    ...
    for multiple in multiple_list:
        ...
        result_dir = f'../../dataset/finetune/finetune_cot_2_2/'
