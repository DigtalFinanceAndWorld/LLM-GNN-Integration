...
from select_analysis import get_analysis, get_gpt_response_by_label
...
def process_node(node, graph, labels, analysis):
    ...
    conclusion = ""
    if label == 0:
        conclusion = "After analysis and discussion, we will temporarily mark it as a Non-Phishing node. The label of this node is 0."
    elif label == 1:
        conclusion = "After analysis and discussion, we will mark it as a Phishing node. The label of this node is 1."
    else:
        print("error label")
...
def main():
    ...
    for multiple in multiple_list:
        ...
        sorted_dataset_list = ["12"]
        print(f"Dataset_list: {sorted_dataset_list}")

        result_dir = f'../../dataset/finetune/finetune_by_label_analysis_logits/'
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i, index in enumerate(sorted_dataset_list):
                analysis_file_path = f"../../prompt/result/gpt-4o-mini/single-expert/{dataset_name}/{multiple}/delay_{delay}/sft_data_by_label/{index}.xlsx"
                analysis_dict = get_gpt_response_by_label(analysis_file_path)
                # 将任务提交到进程池
