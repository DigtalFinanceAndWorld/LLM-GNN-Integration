...
def get_output_label_confidence_vllm(llm, messages):
    ...
    try:
        # 找到第一次出现 "### Analysis" 的位置
        analysis_occurrence = generated_text.find("### Analysis")
        if analysis_occurrence == -1:
            raise ValueError("First '### Analysis' not found.")
    
        # 提取 "### Analysis" 之前的内容
        output_section = generated_text[:analysis_occurrence].strip()
    
        # 将字符串解析为字典
        result = json.loads(output_section)
        score = 0
        if "Label" in result:
            if result["Label"] == "Non-Phishing":
                score = 0
            elif result["Label"] == "Phishing":
                score = 100
            elif "Fraudulent Probability" in result:
                score = int(result["Fraudulent Probability"])
            else:
                raise ValueError("Missing 'Fraudulent Probability' in the parsed output.")
        else:
            raise ValueError("Missing 'Label' in the parsed output.")
