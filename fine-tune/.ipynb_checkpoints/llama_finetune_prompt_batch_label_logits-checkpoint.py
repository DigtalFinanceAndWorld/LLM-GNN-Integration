...
def get_output_label_confidence(messages):
    # 清理 GPU 缓存
    ...
    # 找到 ### Output Label 后的第一个有效字符
    label_start = generated_text.find("### Output Label") + len("### Output Label")
    if label_start >= len(generated_text):
        raise ValueError("### Output Label not found or no character after it.")
    ...
    if label_start >= len(generated_text):
        print("No non-whitespace character found after ### Output Label.")
        return generated_text, "", 0

    # 提取第一个有效字符
    first_char = generated_text[label_start]
    print(f"First Character: '{first_char}' at position {label_start}")
...
"""
    decoded_text, first_char, confidence_score = get_output_label_confidence(messages)
    return decoded_text, first_char, confidence_score
...
def main():
    ...
    # 开始测试 保存测试结果
    # 创建一个空的 DataFrame
    df_results = pd.DataFrame(columns=['index', 'node_address', 'is_phishing', 'label', 'confidence_score', 'messages', 'response'])
    ...
    for i, node in enumerate(graph_test.nodes):
        ...
        if len(test_node_detail) > 60000:
            test_node_detail = test_node_detail[:60000]
        print(f"len test_node_detail: {len(test_node_detail)}")
        
        start_time_A = time.time()
        generated_text, first_char, confidence_score = single_expert_model(test_node_detail, message_task_A,
