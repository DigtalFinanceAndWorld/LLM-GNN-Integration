...
def get_output_label_confidence_vllm(llm, messages):
    ...
    logprob_data = output[0].outputs[0].logprobs[6]
    print(f"Label Logprob Data: {logprob_data}")

    # 提取第一个 token 及其 logprob
    Label_token_id, Label_logprob_obj = next(iter(logprob_data.items()))  # 提取第一个条目

    # 获取解码后的 token 和 log probability
    decoded_token = Label_logprob_obj.decoded_token.strip()
    logprob_value = Label_logprob_obj.logprob

    # 将 logprob 转换为标准概率
    probability = math.exp(logprob_value)

    # 打印结果
    print(f"Label Token: {decoded_token}")
