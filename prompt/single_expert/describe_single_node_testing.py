...
def describe_single_node_test(graph, labels, node):
    ...
    return node_description, is_phishing, index_str
...
if __name__ == '__main__':
    ...
    graph_test_file_path = f'{dataset_dir}/3/train.pkl'
    labels_test_file_path = f'{dataset_dir}/3/train_labels.json'
    graph_test = load_graph(graph_test_file_path)
    labels_test = load_json(labels_test_file_path)

    for i, node in enumerate(graph_test.nodes):
        train_node_detail, is_phishing, _ = describe_single_node_test(graph_test, labels_test, node)
