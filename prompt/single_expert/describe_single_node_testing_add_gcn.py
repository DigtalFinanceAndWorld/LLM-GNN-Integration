...
def describe_single_node_test(graph, labels, node, gnn_pred):
    # 获取节点与索引的映射关系以及节点度中心性
    index_mapping = create_index_mapping(graph)  # 假设该函数返回节点与索引的映射
    degree_centrality = calculate_degree_centrality(graph)  # 假设该函数返回度中心性
    ...
    # 内部节点GNN预测
    gnn_min = min(gnn_pred)
    gnn_max = max(gnn_pred)
    gnn_mean = sum(gnn_pred) / len(gnn_pred)
    gnn_median = gnn_pred.quantile(0.5)
    gnn_25 = gnn_pred.quantile(0.25)
    gnn_75 = gnn_pred.quantile(0.75)
    ...
    node_description = (
        ...
        f"{transaction_details}"
        f"{internal_transaction_details}"
        f"**Prediction of Internel Nodes Phishing Probability By GCN**\n"
        "The GCN model has predicted the phishing probability for the internal nodes as follows:\n"
        f"- Mean Phishing Probability represents the average predicted probability of phishing activity across all internal nodes, which is {gnn_mean:.3f}.\n"
        f"- Median Phishing Probability indicates the middle value of the predicted probabilities, with half of the nodes having probabilities below {gnn_median:.3f}.\n"
        f"- The 75% Phishing Probability represents the probability below which 75% of the nodes fall, and it is {gnn_75:.3f}.\n"
        f"- Max Phishing Probability represents the highest predicted probability among the internal nodes, which is {gnn_max:.3f}."
    )
...
if __name__ == '__main__':
    ...
    gnn_file_path = f'../../baselines/result/ZipZap/delay_5/GCN/result_8.csv'
