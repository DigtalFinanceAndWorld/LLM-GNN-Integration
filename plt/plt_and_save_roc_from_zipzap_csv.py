...
def plot_and_save_roc(result_dir, model_name, total_partitions, start_time, end_time):
    ...
    for index, row in data.iterrows():
        tpr_str = row['TPR']
        try:
            # 预处理字符串，去除 'array(' 和 ')'，并转换为标准列表格式
            tpr_str_cleaned = re.sub(r'array\(', '[', tpr_str).replace(')', ']')
            # 将字符串转换为列表
            tpr_values = ast.literal_eval(tpr_str_cleaned)[0]  # 解析并获取第一个 array 数组
...
def main():
    ...
    # result_dir = '/home/a/zmb_workspace/product/Phisher_detect/baselines/result/MulDiGraph/delay_5/ZipZap'
    # total_partitions = 18
