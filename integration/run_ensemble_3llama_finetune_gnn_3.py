...
def main():
    ...
    finetune_type = "finetune_cot_3_smote"
    gnn_model = "GraphSAGE"

    model_name = f'ensemble_{gnn_model}_{finetune}_{finetune_type}_{strategy}_{multiple1}_{multiple2}_{multiple3}_{agent}'
    ...
    for i in range(2, total_partitions + 1):
        ...
        result_llm_3 = pd.read_excel(
            f'../../prompt/result/{finetune}/{finetune_type}/{agent}/{dataset_name}/{multiple3}/delay_{delay}/{strategy}/{str(i)}.xlsx')
        ...
        pred_llm_3 = result_llm_3[['node_address', 'label', 'confidence_score']]
        pred_llm_3['llm_pred'] = pred_llm_3.apply(
            lambda x: x['label'] * x['confidence_score'] + (1 - x['label']) * (1 - x['confidence_score']), axis=1
        )
        pred_llm_3 = pred_llm_3[['node_address', 'llm_pred']]
        ...
        pred['predicted_prob'] = (pred['gnn_pred'] + pred['llm_pred_1'] * pred['llm_weight1'] + pred['llm_pred_2'] *
                                  pred[
                                      'llm_weight2'] + pred['llm_pred_3'] * pred['llm_weight3']) / (
                                         1 + pred['llm_weight1'] + pred['llm_weight2'] + pred['llm_weight3'])
