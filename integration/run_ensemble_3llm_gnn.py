...
def main():
    ...
    model_name = f'ensemble_{gnn_model}_{finetune}_{strategy}_{multiple1}_{multiple2}_{multiple3}_{agent}'
    result_dir = f'../result/{dataset_name}/delay_{delay}/{model_name}_4~11'
    ...
    for i in range(4, 11):
        result_llm_1 = pd.read_excel(
            f'../../prompt/result/{finetune}/{agent}/{dataset_name}/{multiple1}/delay_{delay}/{strategy}/{str(i)}.xlsx')
        result_llm_2 = pd.read_excel(
            f'../../prompt/result/{finetune}/{agent}/{dataset_name}/{multiple2}/delay_{delay}/{strategy}/{str(i)}.xlsx')
        result_llm_3 = pd.read_excel(
            f'../../prompt/result/{finetune}/{agent}/{dataset_name}/{multiple3}/delay_{delay}/{strategy}/{str(i)}.xlsx')
        ...
        node_map_3 = json.load(
            open(f'../../dataset/clustered_graphs/{dataset_name}/{multiple3}/delay_{delay}/{str(i)}/test_nodemap.json'))
        ...
        pred_llm_1 = result_llm_1[['node_address', 'score']]
        ...
        pred_llm_1 = pred_llm_1[(pred_llm_1['llm_pred_1'] >= 0) & (pred_llm_1['llm_pred_1'] <= 100)].reset_index(drop=True)
        pred_llm_1['cid1'] = pred_llm_1['cid1'].apply(lambda x: int(x.split('_')[1]))
        pred_llm_1['llm_pred_1'] = normalize(pred_llm_1['llm_pred_1'])

        pred_llm_2 = result_llm_2[['node_address', 'score']]
        ...
        pred_llm_2 = pred_llm_2[(pred_llm_2['llm_pred_2'] >= 0) & (pred_llm_2['llm_pred_2'] <= 100)].reset_index(drop=True)
        pred_llm_2['cid2'] = pred_llm_2['cid2'].apply(lambda x: int(x.split('_')[1]))
        pred_llm_2['llm_pred_2'] = normalize(pred_llm_2['llm_pred_2'])

        pred_llm_3 = result_llm_3[['node_address', 'score']]
        pred_llm_3.columns = ['cid3', 'llm_pred_3']
        pred_llm_3['llm_pred_3'] = pred_llm_3['llm_pred_3'].apply(lambda x: check_valid(x))
        pred_llm_3 = pred_llm_3[(pred_llm_3['llm_pred_3'] != 'Invalid')]
        pred_llm_3 = pred_llm_3[(pred_llm_3['llm_pred_3'] >= 0) & (pred_llm_3['llm_pred_3'] <= 100)].reset_index(
            drop=True)
        pred_llm_3['cid3'] = pred_llm_3['cid3'].apply(lambda x: int(x.split('_')[1]))
        pred_llm_3['llm_pred_3'] = normalize(pred_llm_3['llm_pred_3'])

        pred_gnn['cid1'] = pred_gnn['id'].apply(lambda x: node_map_1[x])
        pred_gnn['cid2'] = pred_gnn['id'].apply(lambda x: node_map_2[x])
        pred_gnn['cid3'] = pred_gnn['id'].apply(lambda x: node_map_3[x])

        pred = pd.merge(pred_gnn, pred_llm_1, on='cid1', how='inner')
        pred = pd.merge(pred, pred_llm_2, on='cid2', how='inner')
        pred = pd.merge(pred, pred_llm_3, on='cid3', how='inner')

        pred = pred[['id', 'gnn_pred', 'llm_pred_1', 'llm_pred_2', 'llm_pred_3', 'true_label', 'cid1', 'cid2', 'cid3']]
        ...
        weight3 = normalize(
            pred.groupby('cid3').apply(lambda x: x['gnn_pred'].sort_values(ascending=False)[:20].mean())).to_dict()
        print(weight1)
        print(weight2)
        print(weight3)
        pred['llm_weight1'] = pred['cid1'].map(weight1)
        pred['llm_weight2'] = pred['cid2'].map(weight2)
        pred['llm_weight3'] = pred['cid3'].map(weight3)
        pred['predicted_prob'] = (pred['gnn_pred'] + pred['llm_pred_1'] * pred['llm_weight1'] + pred['llm_pred_2'] * pred[
            'llm_weight2'] + pred['llm_pred_3'] * pred['llm_weight3']) / (1 + pred['llm_weight1'] + pred['llm_weight2'] + pred['llm_weight3'])

        # pred['predicted_prob'] = normalize(pred['gnn_pred'] + 0.33 * pred['llm_pred_1'] + 0.33 * pred['llm_pred_2'] + 0.33 * pred['llm_pred_3'])
        # pred['predicted_prob'] = normalize(pred['gnn_pred'] + 1 * pred['llm_pred_1'] + 1 * pred['llm_pred_2'] + 1 * pred['llm_pred_3'])
