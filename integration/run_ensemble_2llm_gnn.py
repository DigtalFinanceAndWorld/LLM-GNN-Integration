...
def main():
    # strategy_list = ["head", "recent", "represent", "history_recent]
    ...
    model_name = f'ensemble_{gnn_model}_{finetune}_{strategy}_{multiple1}_{multiple2}_{agent}'
    ...
    for i in range(1, total_partitions + 1):
        ...
        pred = pred[['id', 'gnn_pred', 'llm_pred_1', 'llm_pred_2', 'true_label']]
        # pred['predicted_prob'] = normalize(pred['gnn_pred'] + 0.5 * pred['llm_pred_1'] + 0.5 * pred['llm_pred_2'])
        pred['predicted_prob'] = normalize(pred['gnn_pred'] + 1 * pred['llm_pred_1'] + 1 * pred['llm_pred_2'])
